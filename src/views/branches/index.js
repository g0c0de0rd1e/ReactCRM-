...
const Restaurants = () => {
  ...
  const goToClone = (row) => {
    dispatch(
      addMenu({
        id: 'branch-clone',
        url: `branch-clone/${row.uuid}`,
        name: t('branch.clone'),
      })
    );
    navigate(`/branch-clone/${row.uuid}`);
  };

  const goToAdd = () => {
    /*if (restourants.length > 3 || !meta.current_page) {
      toast.error(
        ...
        return (
          <Space>
            <Button
              type='primary'
              icon={<EditOutlined />}
              disabled={row.deleted_at || isDemo ? !row.parent_id : null}
            ...
            <Button
              ...
              disabled={row.deleted_at || isDemo ? !row.parent_id : null}
            />
            {user?.role !== 'manager' && row.parent_id ? (
              <DeleteButton
      ...
      <Table
        ...
        rowSelection={{
          getCheckboxProps: (record) => ({
            disabled: !record.parent_id ? true : false, // Column configuration not to be checked
          }),
          selectedRowKeys: id,
          onChange: (key) => {
            setId(key);
